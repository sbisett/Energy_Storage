...
Candidates = xlsread('C:\Users\sethb\Desktop\Energy Storage\Candidates\Salt\Heat storage candidates matrix SALT');
LossRateTargetArray = Candidates(:,1);
...
Power = PowerArray(CandidateIteration);
StorageTime = StorageTimeArray(CandidateIteration)*3600; %s, storage time
LossRateTarget = LossRateTargetArray(CandidateIteration); %[%]
InsulationThickness = 0; %[m]
P_in                = Power;             % J/s           rate at which THERMAL power is transferred in when charging
P_out               = Power;             % J/s           rate at which THERMAL power is transferred out when discharging
...
%     36000 ...
%     38400];
dt                      = 30;              % s             time step in seconds - I haven't tested convergence, could possibly jack this way up
...
LossRate = HeatLostperHour/MaxHotEnth*100; % percent/hr

disp(['Candidate: ', num2str(CandidateIteration)])
disp(['Time ',num2str(StorageTimeArray(CandidateIteration))])
disp(['Insulation Thickness: ',num2str(Tinsul)])
disp(['%loss/hr: ',num2str(LossRate)])

if (LossRate - LossRateTarget < 0)
    condition = false;
end

InsulationThicknessReport(CandidateIteration) = Tinsul; 
RealLossRateReport(CandidateIteration) = LossRate;
